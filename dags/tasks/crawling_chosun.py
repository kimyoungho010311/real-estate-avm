...
def crawl_chosun(driver_path, max_page, bucket_name, prefix):
    """
    조선일보 부동산 뉴스 크롤링 태스크
    """
    ...
    driver.quit()
    df = pd.DataFrame([
        {"url": url, "date": data["date"], "content": data["content"], "publisher": "조선일보"}
        for url, data in article.items()
    ])

    if df.empty:
        print("No articles found for chosun.")
        return ''
