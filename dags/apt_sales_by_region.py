...
download_dir = "tmp/apt_sales_by_region"
...
with DAG(dag_id='apt_sales_by_region',
        ...
        description='(월) 지역별 아파트 매매 데이터를 수집하는 DAG입니다.)',
        start_date=datetime(2020,2,2),
        schedule='@weekly', # 매주 월요일 00:00시에 실행
        catchup=False,
        ...
        if response.status_code == 200:
            ...
            if 'SttsApiTblData' in data:
                ...
                for i, item in enumerate(rows, start=1):
                    ...
                    print(f"    TIME        : {item.get('WRTTIME_DESC')}")
                    print(f"    DATA_VALUE  : {item.get('DTA_VAL')}")
                    print("-----------------------------")
            else:
    ...
    fetch_apt_sales_by_region_task = fetch_apt_sales_by_region()
    save_df_to_s3_task = save_df_to_s3(fetch_apt_sales_by_region_task)

    fetch_apt_sales_by_region_task >> save_df_to_s3_task